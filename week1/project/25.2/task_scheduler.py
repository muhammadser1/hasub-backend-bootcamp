...
def find_empty_spot(days, name, duration):
    ...
    for i in range(len(days)):
        ...
        if max_count >= duration:
            ...
            return "Task Population Confirmation"
    return "Task Population failed"
...
def fill_specific_time(day, duration, start, days, name):
    if start + duration>=len(days[day]):
        return "failed2"
    j = 0
...
if __name__ == "__main__":
    ...
    for i in range(2):
        ...
        if type_input == 2:  # no specific day,hour
            flag=find_empty_spot(days, name, duration)
            if flag=="Task Population failed":
                print("Task Population failed, No empty spot found for the task. try again\n")
                i=i-1
                continue
        if type_input == 1:
            type_input_is_one(days,name,duration)
